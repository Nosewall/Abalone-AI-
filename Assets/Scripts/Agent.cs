...
public class Agent : MonoBehaviour
{
  ...
  private State AlphaBeta(State currentState)
  {
      ...
      miniMaxState v = MaxValue(currentState, ref largeNegative,ref  largePositive,ref best);
  ...
  private miniMaxState MaxValue(State currentState, ref int alpha, ref int beta, ref State best)
  {

      if (!currentState.getNextStates().Any())
      {
          return new miniMaxState(evaluate(currentState),currentState);
      }
      int v = largeNegative;
      miniMaxState values = null;

      foreach (State nextState in currentState.getNextStates())
      {
          values = MinValue(nextState,ref alpha,ref beta, ref best);
          int min = values.getValue();
          if (min > v)
          {
              v = min;
              values = new miniMaxState(v,currentState);
          }
  ...
  private miniMaxState MinValue(State currentState,ref  int alpha,ref  int beta, ref State best)
  {
      ...
      foreach (State nextState in currentState.getNextStates())
      {   
          values = MaxValue(nextState,ref alpha,ref  beta, ref best);

          int max = values.getValue();
          if (max <= v)
          {
              v = max;
              values = new miniMaxState(v,currentState);
  ...
  */
  ...
  static List<State> generate(State state)
